...
public class throwingEnemy : MonoBehaviour {
    ...
    private void move()
    {
        ...
        if (Vector2.Distance(self.position, character.position) < 15.0f && Mathf.Abs(self.position.x - character.position.x) > .5f)
        {
            hastarget = true;
    ...
    private void strike()
    {
        ...
        {
            strikeTimer = 0;
            if (Vector2.Distance(self.position, character.position) < 15.0f)
            {
